...
SELECT * FROM store WHERE main_cat=1;
-- -----------------------------------------------------
...
INSERT INTO `baemin`.`delivery` (`store_no`, `deli_price`, `deli_area`) VALUES (1, 1234, '1234');
INSERT INTO `baemin`.`delivery` (`store_no`, `deli_price`, `deli_area`) VALUES (1, 0, '0');
select * from delivery where store_no;
...
-- Table `baemin`.`order`
-- -----------------------------------------------------
DROP TABLE IF EXISTS `baemin`.`order` ;

CREATE TABLE IF NOT EXISTS `baemin`.`order` (
  ...
  INDEX `fk_order_cart1_idx` (`cart_no` ASC) ,
  PRIMARY KEY (`odr_no`),
...
ALTER TABLE baemin.order CONVERT TO character SET utf8;
